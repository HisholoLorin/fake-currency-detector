...
def index():
    if request.method == 'POST':
        ...
        if file:
            ...
            cv2.imwrite('app/inputs/input.jpg', image1)
            contour=process('app/inputs/input.jpg')
            if contour==0:
                return render_template('index.html',note=2)  # Assuming process function saves output as 'outputs/output.jpg'
            processed_image = cv2.imread('app/outputs/output.jpg')
            check_part1=processed_image[1:139, 847:1425] #(862, 13),(1406, 129) #(847, 1), (1425, 139)
            cv2.imwrite('app/note_checking/check1.jpg',check_part1)
            note1_result=check('app/note_checking/check1.jpg',t1=100,t2=200,mode=1)
            check_part2=processed_image[410:552, 1093:1397] #(1093, 410), Bottom-right (1397, 552)
            cv2.imwrite('app/note_checking/check2.jpg',check_part2)
            note2_result=check('app/note_checking/check2.jpg',t1=158,t2=240,mode=2)
            if note1_result==0 or note2_result==0 :
                ...
                cv2.imwrite('app/sub/sub5.jpg',sub_images[4])
                cv2.imwrite('app/sub/sub6.jpg',sub_images[5])
                ...
                model1=load_model('app/Model1.h5')
                cv2.imwrite('app/sub/sub1.jpg',sub_images[0])
                img=image.load_img('app/sub/sub1.jpg',target_size=(144,66))
                ...
                model2=load_model('app/Model2.h5')
                cv2.imwrite('app/sub/sub2.jpg',sub_images[1])
                img=image.load_img('app/sub/sub2.jpg',target_size=(100,50))
                x=image.img_to_array(img)
                x=np.expand_dims(x,axis=0)
                images=np.vstack([x])
                ...
                model3=load_model('app/Model3.h5')
                cv2.imwrite('app/sub/sub3.jpg',sub_images[2])
                original_image = cv2.imread('app/sub/sub3.jpg', cv2.IMREAD_GRAYSCALE)
                ...
                cv2.imwrite('app/sub/sub7.jpg',binary_image_rgb)
                ...
                model4=load_model('app/Model4.h5')
                cv2.imwrite('app/sub/sub4.jpg',sub_images[3])
                img=image.load_img('app/sub/sub4.jpg',target_size=(250,60))
                x=image.img_to_array(img)
                x=np.expand_dims(x,axis=0)
                images=np.vstack([x])
                ...
                predictions.append(textinfo('app/sub/sub5.jpg'))
                ...
                if request.files.get('mode') == "Mobile":
                    return {"file" : f"{request.base_url}static/{randomString}.jpg", "prediction" : final}
                return render_template('result.html', prediction=final, image_path=f"{request.base_url}static/{randomString}.jpg")
